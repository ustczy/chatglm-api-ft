...
def read_jsonl(path, max_seq_length, prompt_key,target_key,skip_overlength=False):
    tokenizer = transformers.AutoTokenizer.from_pretrained(chatglm_model_path, trust_remote_code=True)
    config = transformers.AutoConfig.from_pretrained(chatglm_model_path, trust_remote_code=True, device_map='auto')
